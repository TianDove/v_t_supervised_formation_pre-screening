...
def objective(trial: optuna.trial.Trial, data_set_dict: dict, writer: SummaryWriter):
    ...
    nly = 3
    dropout = 0.0
    hid = trial.suggest_categorical('model-n_hid', [16, 32, 64, 128, 256, 512, 1024, 2048])
    MODEL = cls_model_define.PE_fixed_EC_transformer_DC_dense
    model_init_dict = {
    ...
    scheduler_para = {
        ...
        'step_size': 10 * len(train_data_set),
        'gamma': 0.95,
        'last_epoch': -1,
        'verbose': False
    }
...
if __name__ == '__main__':
    ...
    ###################################################################################################################
    # path set up
    workspace_base = 'C:\\MyFile\\python_workspace\\wx'
